from yolov4_gesture.models import Darknet    # YOLOv4网络结构的实现
from yolov4_gesture.utils.datasets import *  # 引入所有数据集处理函数
from yolov4_gesture.utils.utils import *     # 引入yolo的计算函数：比如iou计算
...
class GestureDetector:
    ...
    def __init__(self):
        ...
        self.mod_file = os.path.join(current_dir, "data/gestures.pt")
        self.names_file = os.path.join(current_dir, "data/gestures.names")

        self.img_size = 640   # 设置预测的图像大小
        # 条件2：网络结构
        self.net = Darknet(cfg=self.cfg_file, img_size=self.img_size)
        # 条件3：加载模型
        state_dict = torch.load(self.mod_file, map_location='cpu')["model"]
